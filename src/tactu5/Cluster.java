...
public class Cluster implements Cloneable, T5Containers {
 ...
 // add notes to the sequence array
	  ...
	  /**
	   * Add an array of notes to cluster internal array.
	   */
	  public void addNotes (Note[] inote) {
	        
			// old length
			 
			 int olen = cluster.length ;
			 
			 cluster=(Note[]) PApplet.expand ( cluster, cluster.length + inote.length ) ;
		    
		    
		    for ( int i = 0; i <  inote.length; i++) {
		    	
		    	cluster[olen+i]=(Note) inote[i].clone();
