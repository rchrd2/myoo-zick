...
public class Sequence implements Cloneable, T5Containers {
	  ...
	  // add notes to the sequence array
	  
	  /**
	   * Add an array of notes to Sequence internal array.
	   */
	 public void addNotes (Note[] inote) {
        
		// old length
		 
		 int olen = sequence.length ;
		 
	    sequence=(Note[]) PApplet.expand ( sequence, sequence.length + inote.length ) ;
	    
	    
	    for ( int i = 0; i <  inote.length; i++) {
	    	
	    sequence[olen+i]=(Note) inote[i].clone();
	    
	    updateTime(inote[i].getDuration());
