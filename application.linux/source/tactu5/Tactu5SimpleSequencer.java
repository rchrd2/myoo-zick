...
public class Tactu5SimpleSequencer extends Thread {
	...
	// **********************************//
	// checking for noteReciver app method
	Method fancyEventMethod;

	// checking for looping sequencer method
	Method loopingEventMethod;

	public Tactu5SimpleSequencer(Object p, InternalSequence s,
			Tactu5Controller o, boolean b) {
		...
		try 
		{
			fancyEventMethod = parent.getClass().getMethod("noteReciver", new Class[] { Note.class });

		} catch (Exception e) {
			System.out.println("ALLERT noteReciver(Note) method is not set!");
		}

		try {
			loopingEventMethod = parent.getClass().getMethod("looping", null);
			...
			if (fancyEventMethod != null) {
				...
				for (int h = 0; h < timeline.getCluster(index).getClusterNumber(); h++) {
					...
					try {
						...
						//System.err.println("Disabling fancyEventMethod() for because of an error.");
						e.printStackTrace();
			...
			if (index >= limit) {
				...
				if (loopingEventMethod != null) {
					try {
						...
						//System.err.println("Disabling fancyEventLoop() for "		+ " because of an error.");
						e.printStackTrace();
						//loopingEventMethod = null;
					}
				}
